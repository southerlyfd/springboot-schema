  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
          ...
          <el-button
            v-if="isAuth('ershouyingyindingdan','新增')"
            type="primary"
            @click="addOrUpdateHandler()"
            round
          >新增</el-button>
          <el-button
            v-if="isAuth('ershouyingyindingdan','删除')"
      ...
      <div class="table-content">
        <el-table
            v-if="isAuth('ershouyingyindingdan','查看')"
            :data="dataList"
            border
            ...
            <el-table-column
                ...
                label="操作">
                <template slot-scope="scope">
                                <el-button v-if="isAuth('ershouyingyindingdan','查看')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                                <el-button v-if="isAuth('ershouyingyindingdan','修改')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                <el-button v-if="isAuth('ershouyingyindingdan','删除')" type="text" icon="el-icon-delete" size="small" @click="deleteHandler(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
                                                                                                                                                                                                <el-pagination
...
export default {
  ...
  methods: {
            payHandler(row){
      this.$storage.set('paytable','ershouyingyindingdan');
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "ershouyingyindingdan/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
